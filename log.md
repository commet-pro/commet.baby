...
- [2026-07-03] FRONTEND players shipped: YouTubeEmbed (youtube-nocookie, COPPA §7.1), AudioPlayer (Media Session API, background) and StoryPlayer toggle in /story/[slug] (#16/#17); mock lullaby.wav so the audio player is demo-testable. Mirrored to brain: [[Frontend_Web]].
- [2026-07-03] FRONTEND plans + checkout (#19): /plans with monthly/annual cycle, bilingual add-on toggle, CheckoutInputSchema validation and Stripe redirect (mock-aware); billingApi (checkout/subscription/portal) + billing mocks. Mirrored to brain: [[Frontend_Web]].
