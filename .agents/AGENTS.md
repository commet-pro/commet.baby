...
| Tag                   | Meaning                                                  |
| --------------------- | -------------------------------------------------------- |
| _(no tag)_            | Same as `visibility/public` — visible in all modes       |
...
## 🧠 Brain & Wiki Sync Rule (CRITICAL)

**MANDATORY SPEC-FIRST RULE:**
Before proposing or writing any code, YOU MUST read the corresponding specification/plan in `projects/commet.baby/` or `concepts/`. If the user asks you to implement a feature, FIRST check the Obsidian Wiki. Nenhuma linha de código deve ser escrita sem uma spec.

**BRAIN SYNC RULE:**
Whenever a project implementation plan is updated, or significant architectural/business/code changes occur, YOU MUST update the corresponding wiki files in `projects/commet.baby/` or the global `concepts/` to maintain the most advanced code standards and semanticity. The Obsidian Wiki (in this repository) is the persistent brain of the project.
